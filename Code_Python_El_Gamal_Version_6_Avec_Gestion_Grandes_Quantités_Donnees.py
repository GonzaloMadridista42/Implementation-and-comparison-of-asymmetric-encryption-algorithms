...
def gen_key(q):
    ...
    return key, q  # Return key and q as a tuple
...
if __name__ == '__main__':
    # Code block that measures the algorithmic complexity of the keys generated and the encryption and decryption process, used to handle a big quantitiy of data (large messages) provided by the Python code implementing the El-Gamal algorithm
    ...
    if 'g' in globals() and 'key' in globals():
        ...
        key_gen_time, key_gen_memory = measure_key_generation(q)
    ...
    for i, encrypted_message in enumerate(encrypted_messages):
        ...
        pt = decryption(encrypted_message, p, key, q)
        ...
        # Display decrypted message as string
        decrypted_message_text = ''.join(pt)
    ...
    encryption_start_time = time.time()
    ct, p, s = encryption(plaintext_message, q, h, g)
    encryption_end_time = time.time()
    encryption_time = encryption_end_time - encryption_start_time
