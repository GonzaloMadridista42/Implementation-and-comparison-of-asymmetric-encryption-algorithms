...
if __name__ == '__main__':
    # Code block that measures the size (length) of the keys generated by the RSA encryption and decryption algorithm, that allows to compare the size of those keys generated by the RSA algorithm   
    ...
    if 'public_key' in globals() and 'private_key' in globals():
        # Print key sizes
        print("\nSize of the public key:", public_key_size*8, "bits") #public_key_size contains the number of characters that contain the clear and plain message, that'll be encrypted and decrypted 
        print("Size of the private key:", private_key_size*8, "bits")
        
        # Check if the size of the public key is greater than the size of the private key
        if public_key_size > private_key_size:
            print("\nThe size of the public key is greater than the size of the private key.")
        elif public_key_size < private_key_size:
            print("\nThe size of the private key is greater than the size of the public key.")
        else:
            print("\nThe size of the public key is equal to the size of the private key.")
        
        # Measure the key generation complexity for encryption
        encrypt_key_gen_time, encrypt_key_gen_memory = measure_key_generation(user_response_key_size)
        
        # Ensure non-zero values for key generation complexity
        encrypt_key_gen_time_str = "{:.8f}".format(max(0.00000001, encrypt_key_gen_time))
        encrypt_key_gen_memory_str = "{:.8f}".format(max(0.00000001, encrypt_key_gen_memory))
        
        print("\nComplexity of the public key generation, for encrypting clear messages:")
        print("Time taken:", encrypt_key_gen_time_str, "seconds")
        print("Memory used:", encrypt_key_gen_memory_str, "KB")

        # Measure the key generation complexity for decryption
        decrypt_key_gen_time, decrypt_key_gen_memory = measure_key_generation(user_response_key_size)
        
        # Ensure non-zero values for key generation complexity
        decrypt_key_gen_time_str = "{:.8f}".format(max(0.00000001, decrypt_key_gen_time))
        decrypt_key_gen_memory_str = "{:.8f}".format(max(0.00000001, decrypt_key_gen_memory))
        
        print("\nComplexity of the private key generation, for decrypting encrypted messages:")
        print("Time taken:", decrypt_key_gen_time_str, "seconds")
        print("Memory used:", decrypt_key_gen_memory_str, "KB")