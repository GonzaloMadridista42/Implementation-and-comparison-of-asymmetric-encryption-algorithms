...
if __name__ == '__main__':
    # Code block that measures the size (length) of the keys generated by the El-Gamal encryption and decryption algorithm, that allows to compare the size of those keys generated by the El-Gamal algorithm
    ...
    if 'g' not in globals() or 'key' not in globals():
        ...
        q = random.randint(pow(2, 900), pow(2, user_response_key_size))
        g = random.randint(2, q)
        key, q, public_key_size, private_key_size = gen_key(q)
        h = power(g, key, q)
    else:
    ...
    if 'g' in globals() and 'key' in globals():
        # Print key sizes
        ...
        encrypt_key_gen_time, encrypt_key_gen_memory = measure_key_generation(q)
        ...
        decrypt_key_gen_time, decrypt_key_gen_memory = measure_key_generation(q)
