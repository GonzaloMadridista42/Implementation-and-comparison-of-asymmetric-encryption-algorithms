...
def decode_message(encoded_message):
    return ''.join(str(char) for char in encoded_message)
...
if __name__ == '__main__':
    # Code block that measures all the execution times associated to the RSA algorithm 
    ...
    # Print/display the encoding time
    print("\nEncoding time: {:.6f} seconds ({:.6f} milliseconds)".format(encoding_time, encoding_time * 1000))
    
    # Measure the decoding time
    ...
    # Print/display the decoding time
    print("\nDecoding time: {:.6f} seconds ({:.6f} milliseconds)".format(decoding_time, decoding_time * 1000))
